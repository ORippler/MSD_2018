...
def augment_gaussian_noise(data, noise_variance=(0, 0.1)):
    ...
    for sample_idx in range(data.shape[0]):
        if noise_variance[0] == noise_variance[1]:
            ...
            variance = random.uniform(noise_variance[0], noise_variance[1])
        rdist = np.random.normal(0, variance, size=data[sample_idx, 0].shape)
        for modal in range(data.shape[1]):
            data[sample_idx, modal] = data[sample_idx, modal] + np.median(data[sample_idx, modal]) * rdist
    return data[0]
